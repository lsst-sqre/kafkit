...
class SubjectCache:
    ...
    def get_id(self, subject: str, version: int) -> int:
        ...
        try:
            ...
            raise ValueError(
                f"Schema with subject {subject!r} version {version!r} "
                "not cached."
            ) from e
    ...
    def get(self, subject: str, version: int | str) -> dict[str, Any]:
        ...
        version : `int`
            ...
            given as a string (``"latest"``), a `TypeError` is raised.
        ...
        ValueError
            Raised if the schema does not exist in the cache.
        TypeError
            Raised if the version is a string, like "latest".
        ...
        if not isinstance(version, int):
            msg = f"version must be an int, got {version}"
            raise TypeError(msg)
        try:
